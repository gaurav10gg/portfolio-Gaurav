      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {educationData.map((edu, index) => (
            ...
            >
              <div className="relative h-full p-6 rounded-2xl bg-dark-800/50 border border-white/10 hover:border-green-400/50 transition-all duration-300 card-glow">
                <div className="absolute top-4 right-4 flex items-center gap-2 text-green-400">
                  <GraduationCap className="w-5 h-5" />
                  <span className="font-bold">{edu.grade}</span>
                  {edu.gradeLabel && <span className="text-xs text-white/50">{edu.gradeLabel}</span>}
                </div>
